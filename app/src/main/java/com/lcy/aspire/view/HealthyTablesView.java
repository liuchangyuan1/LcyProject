...
public class HealthyTablesView extends View {
    ...
    public HealthyTablesView(Context context, AttributeSet attrs,
        ...
        TypedArray array = context.getTheme().obtainStyledAttributes(attrs,
                R.styleable.HealthyTablesView, defStyleAttr, 0);
        int index = array.getIndexCount();
        for (int i = 0; i < index; i++) {
            int attr = array.getIndex(i);

            switch (attr) {
                case R.styleable.HealthyTablesView_coordinatesLineWidth:
                    // 这里将以px为单位,默认值为2px;
                    ...
                    break;
                case R.styleable.HealthyTablesView_coordinatesTextColor:
                    mCoordinatesTextColor = array.getColor(attr, Color.BLACK);
                    break;
                case R.styleable.HealthyTablesView_coordinatesTextSize:
                    mCoordinatesTextSize = array.getDimensionPixelSize(attr,
                            (int) TypedValue.applyDimension(
                                    TypedValue.COMPLEX_UNIT_SP, 11,
                                    getResources().getDisplayMetrics()));
                    break;
                case R.styleable.HealthyTablesView_lineColor:
                    mLineColor = array.getColor(attr, Color.BLUE);
                    break;
                case R.styleable.HealthyTablesView_averageCircleRadius:
                    mCircleradius = array.getDimensionPixelSize(attr,
                            (int) TypedValue.applyDimension(
                                    TypedValue.COMPLEX_UNIT_DIP, 10,
                                    getResources().getDisplayMetrics()));
                    break;
                case R.styleable.HealthyTablesView_bgColor:
                    mBgColor = array.getColor(attr, Color.WHITE);
                    break;
                case R.styleable.HealthyTablesView_lineWidth:
                    mLineWidth = array.getDimensionPixelSize(attr,
                            (int) TypedValue.applyDimension(
                                    TypedValue.COMPLEX_UNIT_DIP, 11,
                                    getResources().getDisplayMetrics()));
                    break;
                case R.styleable.HealthyTablesView_maxCircleColor:
                    mMaxcircleColor = array.getColor(attr, Color.GREEN);
                    break;
                case R.styleable.HealthyTablesView_minCircleColor:
                    mMincircleColor = array.getColor(attr, Color.WHITE);
                    break;
                case R.styleable.HealthyTablesView_tableType:
